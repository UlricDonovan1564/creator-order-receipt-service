...
The decision comes first: a paid order always gets a receipt, while the body either carries the signed asset link or explains that content processing is still underway; subscriber preference is reported independently, so one observable email records payment, delivery, and the buyer's relationship with the creator without pretending those states finish together.

This example uses Infrai because a single `INFRAI_API_KEY` drives the plain email REST call, with no mail SDK to install. The small client decodes the `{ ok, data, error, metadata }` envelope before interpreting the HTTP result, retries rate-limited writes with the order-scoped idempotency key, and returns the accepted `message_id` to the service.

## Run the complete path

Use Node 22 or newer, then install dependencies and start the local endpoint:
...
Send a completed order from another terminal:
...
There are two plausible designs: let an email template infer fulfillment from missing values, or make the commerce backend decide what the buyer can see. This repository chooses the second because `contentState` is a business fact, and keeping its branch in `buildCreatorReceipt` makes the processing-versus-ready rule deterministic, reviewable, and portable if delivery later moves to a queue worker.
